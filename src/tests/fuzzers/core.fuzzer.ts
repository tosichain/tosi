...
async function run(): Promise<void> {
  ...
  const log = new Logger("core-fuzzer", "debug");
  const engine = new Engine(config.engine, log);

  const coordinator = new CoordinatorRPC({ serverAddr: config.coordinatorAddr, tls: false });
  ...
  log.info("fuzzing finished", undefined, {
    txnStats: engine.getTransactionStats(),
  });
}
...
async function generateTransactions(
  ...
  while (currentUnixTime() - startTime < duration) {
    const txnCount = config.txnCountMin + Math.random() * (config.txnCountMax - config.txnCountMin);
    const txns = await engine.generateTxns(txnCount);
    for (const txn of txns) {
      coordinator.submitSignedTransaction(txn);
    }
