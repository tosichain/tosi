...
const emptyDataHashes = [
  Buffer.from(ethers.utils.arrayify("0x011b4d03dd8c01f1049143cf9c4c817e4b167f1d1b83e5c6f0f10d89ba1e7bce")),
];
 ...
 */
export function getEmptyTreeHash(level: number): Buffer {
 ...
 * @return the Merkle root hash
 */
export function computeMerkleRootHashFromHashes(hashes: Buffer[], level: number, rootLevel: number): Buffer {
  if (hashes.length === 1 && level === rootLevel) {
    ...
    for (let i = 0; i < hashes.length; i += 2) {
      ...
      const hash = keccak256(Buffer.concat([hash1, hash2]));
 ...
 * @return the Merkle root hash
 */
export function computeMerkleRootHash(buffer: Buffer, log2Size: number): Buffer {
  let bufferLog2Size = Math.max(3, log2Size);
